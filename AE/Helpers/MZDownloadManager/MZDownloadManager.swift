            ...
            if downloadTask.isEqual(downloadModel.task) {
                ...
                if fileManager.fileExists(atPath: basePath) {
                    ...
                    do {
                        ...
                        DownloadPreference.shared.updateStatus(status: .DOWNLOADED, fileName: String(fileName))
                      if /fileName.contains(FileName.OBJECT_FILE.rawValue) {
                        presentNotificationForDownload(userInfo: ["fileName" : String(fileName)], title: VCLiteral.DOWNLOADS_TITLE_NOTIFICATION.localized, body: VCLiteral.DOWNLOAD_BODY.localized)
...
extension MZDownloadManager {
  ...
  @objc public func presentNotificationForDownload(userInfo: [AnyHashable : Any], title: String?, body: String?) {
...
//    if applicationState == UIApplication.State.background {
      ...
      let localNotification = UNNotificationRequest(identifier: "DOWNLOAD_COMPLETED", content: content, trigger: nil)
